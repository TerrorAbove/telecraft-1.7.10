...
import com.teleCraftMod.entity.EntityLandMinePrimed;
...
public class BlockLandMine extends BlockBasePressurePlate {
	...
	private List<EntityLandMinePrimed> instances;
	...
	public BlockLandMine(String str, Material m)
	{
		...
		instances = new ArrayList<EntityLandMinePrimed>();
	}
	...
	public int func_150065_e(World world, int x, int y, int z)
	{
		...
		if(triggered)
		{
			Iterator<EntityLandMinePrimed> iter = instances.iterator();
			
			while(iter.hasNext())
			{
				EntityLandMinePrimed mine = iter.next();
				
				if(mine.worldObj == world && (int)mine.posX == x && (int)mine.posY == y && (int)mine.posZ == z)
				{
	...
	public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase entity, ItemStack stack)
	{
		...
		instances.add(new EntityLandMinePrimed(world, x, y, z, entity));
	}
